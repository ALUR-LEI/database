...
%schema_spy% -t "%common_dir%pgsql.properties" -host %host% -port %port% -db %dbname% -u %username% -p %pword% -o "%doc_path%" -dp %common_dir%postgresql-42.2.5.jar -schemas "address,administrative,application,bulk_operation,cadastre,document,opentenure,party,source,system,transaction,public" -gv %graph_vis% -hq -I "(.*_historic)" -X "(.*.rowidentifier)|(.*.rowversion)|(.*.change_action)|(.*.change_user)|(.*.change_time)" -ahic -norows -noimplied -meta "%common_dir%\metafiles" >> %DICT_LOG% 2>&1
