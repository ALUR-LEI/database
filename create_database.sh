...
# and load it with configuration data. The script also
# de-compresses and loads a test dataset (currently empty).
#
...
$zip_exe e -y -o"$data_path" "$data_path/emptySOLAdatabase.7z" >> $BUILD_LOG 2>&1
# Use -p option if the archive is password protected as follows
# $zip_exe e -y -p$archive_password -o"$data_path" "$data_path/emptySOLAdatabase.7z" >> $BUILD_LOG 2>&1
